...
├── ios/            The Swift/SwiftUI app: voice pipeline, App Intents, CarPlay scene
...
Louis doesn't own a Mac; only device access is an iPhone 15 Pro. The whole dev loop is built around that constraint and stays free through Phase 5 of the roadmap — see [docs/dev-workflow.md](docs/dev-workflow.md) for the full explanation. Short version: edit here → push to GitHub → a free macOS CI runner builds it → install on the iPhone via [SideStore](https://docs.sidestore.io) (free Apple ID sideloading, no TestFlight, no $99/year fee until Phase 6/CarPlay).
